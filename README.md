...
```sh nom_du_programme [options] arguments```
...
```sh firefox nomDuSite &``` (ex: ```sh firefox www.google.ca &```)
...
```sh pwd``` : **p**rint **w**orking **d**irectory
...
- Vous pouvez obtenir de l'aide pour la plupart des programmes avec les options `--help` et/ou `-h`. Par exemple : ```sh ls --help``` affichera les options communes de la commande ```sh ls```
